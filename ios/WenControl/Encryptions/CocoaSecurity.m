...
+ (CocoaSecurityResult *)aesEncrypt:(NSString *)data key:(NSData *)key iv:(NSData *)iv
{
    ...
    bodyData = [self paddingData:bodyData];
    
    return [self aesEncryptWithData:bodyData key:key iv:iv];

}

+(NSData *)paddingData:(NSData *)data{
    NSArray *array = [NSData dataToByte:data];
    Byte *byte = (Byte *)[data bytes];
    int a  = array.count % 16;
    if (a == 0) {
        return data;
    }
    
    Byte paddingBytes[array.count+16-a];
    for (NSInteger i = 0 ; i < array.count+16-a; i++) {
        if (i < array.count) {
    ...
    NSData *returnData = [NSData dataWithBytes:paddingBytes length:array.count+16-a];
    return returnData;
}
